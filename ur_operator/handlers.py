...
def init_uptimerobot_api(logger):
    ...
    try:
        ...
        logger.error(f'failed to create UptimeRobot API')
        raise e
...
def update_monitor(logger, identifier, **kwargs):
    ...
    if resp['stat'] == 'ok':
        ...
        raise kopf.PermanentError(
            f'failed to update monitor with ID {identifier}: {resp["error"]}')
...
def get_identifier(status: dict):
    ...
    elif on_create.__name__ in status:
...
def on_create(name: str, spec: dict, logger, **kwargs):
...
def on_ingress_create(name: str, namespace: str, spec: dict, logger, **kwargs):
    index = 0
    for rule in spec['rules']:
        ...
        monitor_obj = k8s.create_k8s_ur_monitor_with_body(
            namespace, monitor_body)
        logger.info(f'created new UptimeRobotMonitor object for URL {url}')
        index += 1


@kopf.on.update('networking.k8s.io', 'v1', 'ingresses')
def on_ingress_update(name: str, namespace: str, spec: dict, diff: list, logger, **kwargs):
    ...
    for rule in spec['rules']:
        ...
        if index >= previous_rule_count:  # at first update existing UptimeRobotMonitors, we currently don't check if there's actually a change
            ...
            monitor_obj = k8s.update_k8s_ur_monitor_with_body(
                namespace, monitor_name, monitor_body)
    ...
    while index < previous_rule_count:  # make sure to clean up remaining UptimeRobotMonitors
        ...
        logger.info(f'deleted obsolete UptimeRobotMonitor object')
        index += 1


@kopf.on.update(crds.group, crds.MonitorV1Beta1.version, crds.MonitorV1Beta1.plural)
def on_update(name: str, spec: dict, status: dict, diff: list, logger, **kwargs):
    try:
        identifier = get_identifier(status)
    except KeyError as e:
        raise kopf.PermanentError(
            "was not able to determine the monitor ID for update")
...
def on_delete(status: dict, logger, **kwargs):
    try:  # making sure to catch all exceptions here to prevent blocking deletion
        identifier = get_identifier(status)
        delete_monitor(logger, identifier)
    except KeyError as e:
        raise kopf.PermanentError(
            "was not able to determine the monitor ID for deletion")
    except Exception as e:
        raise kopf.PermanentError(f"deleting monitor failed: {e}")
