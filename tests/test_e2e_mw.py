...
def create_k8s_ur_mw(namespace, name, wait_for_seconds=1, **spec):
    k8s.create_k8s_crd_obj(crds.MaintenanceWindowV1Beta1, namespace, name, **spec)
    time.sleep(wait_for_seconds)


def update_k8s_ur_mw(namespace, name, wait_for_seconds=1, **spec):
    k8s.update_k8s_crd_obj(crds.MaintenanceWindowV1Beta1, namespace, name, **spec)
    time.sleep(wait_for_seconds)


def delete_k8s_ur_mw(namespace, name, wait_for_seconds=1):
